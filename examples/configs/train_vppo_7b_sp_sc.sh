...
EXP_NAME="vppo_7b_sp0.02_then_sc0.01_pen0.06"
...
# Epoch 1: Only use SP (Semantics-Preserving) KL reward
echo "Starting Epoch 1: SP KL reward only"
...
# Epoch 2: Only use SC (Semantics-Changing) KL reward
echo "Starting Epoch 2: SC KL reward only"
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python3 -m verl.trainer.main \
    config=${CONGI_FILE} \
    data.train_files=${TRAIN_FILE} \
    data.val_files=${VAL_FILE} \
