...
EXP_NAME="vppo_7b_sc0.01_then_sp0.01_pen0.06"
...
# Epoch 1: Only use SC (Semantics-Changing) KL reward
echo "Starting Epoch 1: SC KL reward only"
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python3 -m verl.trainer.main \
    ...
    algorithm.use_sp_kl_reward=False \
    algorithm.sp_kl_coef=0.01 \
...
# Epoch 2: Only use SP (Semantics-Preserving) KL reward
echo "Starting Epoch 2: SP KL reward only"
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python3 -m verl.trainer.main \
    config=${CONGI_FILE} \
    data.train_files=${TRAIN_FILE} \
    data.val_files=${VAL_FILE} \
    ...
    algorithm.sp_kl_coef=0.01 \
    algorithm.use_sc_kl_reward=False \
    algorithm.sc_kl_coef=0.01 \
    algorithm.use_advantage_shaping=False \
    algorithm.use_entropy_penalty=True \
    algorithm.top_p_perception_tokens=${top_p_perception_tokens} \
