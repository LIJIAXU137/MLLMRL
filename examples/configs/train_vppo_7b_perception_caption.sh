...
EXP_NAME="vppo_7b_perception0.01_then_caption0.01_pen0.06"
...
# Epoch 1: Only use perception kl loss
echo "Starting Epoch 1: Perception KL loss only"
...
# Epoch 2: Only use caption kl loss
echo "Starting Epoch 2: Caption KL loss only"
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python3 -m verl.trainer.main \
    config=${CONGI_FILE} \
    data.train_files=${TRAIN_FILE} \
    data.val_files=${VAL_FILE} \
    ...
    algorithm.perception_kl_coef=0.01 \
    algorithm.use_caption_kl_loss=True \
    algorithm.caption_kl_coef=0.01 \
    algorithm.use_advantage_shaping=False \
    algorithm.use_entropy_penalty=True \
    algorithm.top_p_perception_tokens=${top_p_perception_tokens} \
